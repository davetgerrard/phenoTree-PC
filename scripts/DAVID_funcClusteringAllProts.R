...
## This version to use ALL PROTEINS with annotations. 
...
if(makeNew) {		# only create binaryGrid and kappaMatrix if none available
	...
	# ? useList <- names(termCounts[termCounts > 3])
	...
	kappaMatrix <- read.delim("C:/Users/dave/HalfStarted/PhenotypeTree/data/ubiProtsKappaMatrixDetected.tab",sep="\t",header=T)
